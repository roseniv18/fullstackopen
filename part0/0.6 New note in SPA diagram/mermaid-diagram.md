...
participant server
	...
	Note right of browser: JavaScript handles the submit event and sends a POST request to the server. The page does NOT reload (because we are using e.preventDefault() when submitting the form).
